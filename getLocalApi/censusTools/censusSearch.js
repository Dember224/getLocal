...
const chamber_translator = function(chamber){
  chamber = chamber.toLowerCase()
  if(chamber.includes(/senate|senator/)){
    return 'upper'
  } else if(chamber.includes(/assembly|house|general|representative/)) {
    return 'lower'
  } else {
    throw new Error('Unfamiliar with this chamber. Needs to be added to the list');
  }
}
...
const searchCensusStatsByDistrict = function(callData, callback){
  search_fips.searchFipsSingleDistrict({state:callData.state, year:callData.year, chamber:callData.chamber, district_number: callData.district_number}, async (e, fips_data)=>{
    ...
    for await (const fips_object of fips_data){
      getCensus({latitude: fips_object.latitude, longitude: fips_object.longitude, district_number:callData.district_number}, async (e, census_data)=>{
        if(e) return e;
